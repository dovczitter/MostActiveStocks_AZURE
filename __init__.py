...
serverPort = 7071
driver = None
packageName='MostActiveStocks_AZURE'
...
def getNavStr():
    ...
    basePath = f'http://{hostName}:{serverPort}/api/{packageName}?market='
   
    navStr = f'<!DOCTYPE html><html><head><title>{packageName}</title></head>'
    # 
...
# http://localhost:7071/api/MostActiveStocks_AZURE?market=nasdaq
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    logging.info(f'parsed_url : {parsed_url}')
    if parsed_url.path == f'/api/{packageName}':
        ...
        if 'market' in params:
            for item in marketList:
                if item[0] == params['market'][0].lower():
                    ...
                    break
        if marketUrl == '':
            return func.HttpResponse(getHomeStr().encode("utf-8"), status_code=200, headers={'content-type':'text/html'})
        else :
            htmlTitle, tableData = getJson(marketUrl, marketName)
            return func.HttpResponse(getDataStr(htmlTitle, tableData).encode("utf-8"), status_code=200, headers={'content-type':'text/html'})
